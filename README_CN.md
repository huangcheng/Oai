...
- **透明无边框、始终置顶**的窗口与角色动画
- **Sprite pack 引擎** —— 通过 `.spk` 包加载自定义角色
...
Seelie 通过 sprite pack（`.spk` 文件）和 Codex 宠物包（`.codex-pet` 文件）支持自定义角色。每个包包含：
...
1. **拖拽**：将 `.spk` 或 `.codex-pet` 文件拖到宠物窗口上
