...
Codex CLI introduced `.codex-pet` files — animated companion characters that react to coding events. Oai currently only supports its native `.opk` character packs (Lottie/SpriteSheet/Live2D). Users want to bring their Codex pets into Oai, but there's no loading path. Adding Codex pet support expands the character ecosystem without requiring users to repack or convert assets.
...
- **Map Codex pet metadata** to Oai's `CharacterPack::Metadata` and `CharacterConfig`
...
### New Capabilities
- `codex-pet-loading`: Discover, load, and parse `.codex-pet` zip archives into Oai's `CharacterPack` data model
