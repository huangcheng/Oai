// Qlippy plugin for OpenCode — maps OpenCode bus events to unified event names
// and sends them to the Qlippy desktop pet via IPC.
//
// Platform transport:
//   Linux / macOS → Unix domain socket  (~/.qlippy/qlippy.sock)
//   Windows       → Named pipe          (\\.\pipe\clippy)

import { sendToQlippy } from './ipc.mjs';
...
export default function qlippyPlugin() {
  return {
    name: 'qlippy',
    ...
    onEvent(event) {
      ...
      sendToQlippy(message).catch(() => {
        // Silently fail — Qlippy might not be running
      });
    },
  };
}
